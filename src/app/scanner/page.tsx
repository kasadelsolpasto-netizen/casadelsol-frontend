...
import { CheckCircle, XCircle, ArrowLeft, Loader2, ShieldAlert, DoorOpen, QrCode, DollarSign, CreditCard, Search } from 'lucide-react';
...
export default function ScannerPage() {
  ...
  const [dniEventId, setDniEventId] = useState('');
      ...
      if (token.startsWith('KASA_SHOP_PAY:') || token.startsWith('KASA_SHOP_DELIVER:')) {
        ...
        if (res.ok) {
          ...
          if (verificationToken && orderData.verification_token && verificationToken !== orderData.verification_token) {
            setStatus({ type: 'shop_error', message: '¡TOKEN INVÁLIDO! Posible fraude.' }); return;
          }
          if (prefix.includes('PAY') && orderData.status === 'PENDING') {
            setStatus({ type: 'shop_order', order: orderData, intent: 'PAY' }); return;
          }
          if (orderData.status === 'DELIVERED') {
            setStatus({ type: 'shop_error', message: 'ERROR: PEDIDO YA ENTREGADO' }); return;
          }
          if (orderData.status === 'PENDING') {
            setStatus({ type: 'shop_error', message: 'ERROR: PAGO PENDIENTE (Cobrar primero)' }); return;
          }
          if (['PAID', 'READY'].includes(orderData.status)) {
            ...
            if (updateRes.ok) { setStatus({ type: 'shop_success', name: orderData.user?.name || 'Invitado', total: orderData.total }); return; }
          }
          setStatus({ type: 'shop_error', message: 'ESTADO DESCONOCIDO: ' + orderData.status }); return;
        }
  ...
  const handleDniSearch = async () => {
    if (!dniInput.trim() || !dniEventId) return;
    setDniStatus({ type: 'loading' });
    setDniInput('');
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/qrs/scan-by-dni`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${getToken()}` },
        body: JSON.stringify({ dni: dniInput.trim(), event_id: dniEventId })
      });
      const data = await res.json();
      if (res.ok) {
        setDniStatus({ type: 'admitted', name: data.attendee_name || 'Asistente', ticket: data.ticket_type?.name || 'Entrada', dni: data.attendee_dni || dniInput });
      } else if (res.status === 400) {
        setDniStatus({ type: 'already_used', message: data.message || 'Entrada ya utilizada.' });
      } else {
        setDniStatus({ type: 'not_found', message: data.message || 'No encontrado.' });
      }
    } catch {
      setDniStatus({ type: 'not_found', message: 'Error de conexión.' });
    }
              ...
              {(status.type === 'shop_error' || status.type === 'already_used' || status.type === 'invalid') && (
                <div className="w-full p-10 rounded-[3rem] border-8 border-red-600 bg-red-600 shadow-[0_0_100px_rgba(220,38,38,0.5)] flex flex-col items-center gap-4 animate-in zoom-in-95 duration-200">
                  ...
                  <h2 className="font-black uppercase tracking-tighter text-white text-4xl italic text-center">
                    {status.type === 'already_used' ? 'YA ENTREGADO' : status.message}
                  </h2>
                </div>
              )}
        ...
        {mode === 'taquilla' && (
          <div className="p-4 max-w-md mx-auto w-full flex flex-col gap-5">
            ...
            <div className="bg-zinc-900/80 border border-zinc-700 rounded-2xl p-5">
              ...
              <select value={dniEventId} onChange={e => { setDniEventId(e.target.value); setDniStatus({ type: 'idle' }); }}
                className="w-full bg-black border border-zinc-700 rounded-xl px-4 py-3 text-sm text-white outline-none mb-3">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={handleDniSearch}
                  disabled={!dniInput.trim() || !dniEventId || dniStatus.type === 'loading'}
            ...
            <div className="bg-zinc-900/80 border border-zinc-700 rounded-2xl p-5">
              ...
              <form onSubmit={handleTaquillaSubmit} className="flex flex-col gap-3">
                <input placeholder="Nombre" value={taqForm.name}
                  onChange={e => setTaqForm({...taqForm, name: e.target.value})}
                  className="bg-black border border-zinc-700 rounded-xl px-4 py-3 text-white outline-none focus:border-orange-400" />
                <input placeholder="Costo" type="number" value={taqForm.amount}
                  onChange={e => setTaqForm({...taqForm, amount: e.target.value})}
                  className="bg-black border border-zinc-700 rounded-xl px-4 py-3 text-white outline-none focus:border-orange-400" />
                {taqError && <p className="text-red-400 text-xs font-bold text-center">{taqError}</p>}
                {taqSuccess && (
                  <div className="bg-neon-green/20 border border-neon-green rounded-xl p-3 text-neon-green text-center font-black text-sm uppercase tracking-widest animate-in zoom-in-95">
                    ✅ Registrado
                  </div>
                )}
                <button type="submit" disabled={taqSaving}
                  className="bg-orange-500 hover:bg-orange-400 text-white py-4 rounded-xl font-black uppercase tracking-widest transition-colors disabled:opacity-50">
