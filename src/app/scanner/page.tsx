...
import { CheckCircle, XCircle, ArrowLeft, Loader2, UserCheck, ShieldAlert, DoorOpen, QrCode, Check, Users, DollarSign, Package, CreditCard, Search } from 'lucide-react';
  ...
  const handleDniSearch = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setDniStatus({
          type: 'admitted',
          name: data.attendee_name || 'Asistente',
          ticket: data.ticket_type?.name || 'Entrada',
          dni: data.attendee_dni || dniInput,
        });
    ...
    // Auto-limpiar después de 6 segundos y poner foco de vuelta
    setTimeout(() => {
      setDniStatus({ type: 'idle' });
        ...
        {mode === 'taquilla' && (
          <div className="p-4 max-w-md mx-auto w-full flex flex-col gap-5">
            ...
            <div className="bg-zinc-900/80 border border-zinc-700 rounded-2xl p-5">
              ...
              <select
                value={dniEventId}
                onChange={e => { setDniEventId(e.target.value); setDniStatus({ type: 'idle' }); }}
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="bg-neon-green text-black font-black px-5 rounded-xl text-sm uppercase tracking-widest hover:bg-neon-green/80 transition-colors disabled:opacity-40 active:scale-95 flex items-center gap-1.5"
                >
                  {dniStatus.type === 'loading'
