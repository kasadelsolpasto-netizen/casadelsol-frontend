...
import { Calendar, MapPin, Share2, MessageCircle, Mail, CheckCircle, XCircle, Ticket, Download } from 'lucide-react';
...
export default function PublicTicketPage({ params }: { params: { token: string } }) {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text: `🎉 ${ticket?.attendee_name} tiene entrada para ${ticket?.event?.title}. Muestra este código QR en la puerta.`,
          url: ticketUrl,
        });
        setShareSuccess('¡Compartido!');
      } catch {}
    }
  };
  ...
  const shareWhatsApp = () => {
    const text = encodeURIComponent(
      `🎶 *Entrada Kasa del Sol*\n\n*${ticket?.event?.title}*\nAsistente: ${ticket?.attendee_name}\n\nPresenta este link en la puerta para obtener tu QR:\n${ticketUrl}`
    );
  ...
  const shareEmail = () => {
    ...
    const body = encodeURIComponent(
      `Hola ${ticket?.attendee_name},\n\nAquí está tu entrada digital para el evento "${ticket?.event?.title}".\n\nPresenta el siguiente link en la puerta para acceder:\n${ticketUrl}\n\n¡Nos vemos en la pista! 🎶\n— Kasa del Sol`
    );
  ...
  if (notFound) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center p-6">
        ...
        <h1 className="text-2xl font-black uppercase tracking-widest text-white mb-2">Ticket No Encontrado</h1>
        <p className="text-zinc-500 text-sm">El enlace no es válido o el ticket no existe.</p>
      </div>
    );
  }

  const eventDate = ticket.event?.date ? new Date(ticket.event.date).toLocaleDateString('es-CO', {
  ...
  return (
    <div className="min-h-screen pb-16 pt-4">
      ...
      <div className="max-w-sm mx-auto px-4">
        ...
        {/* Status badge */}
        <div className={`flex items-center justify-center gap-2 mb-6 py-2 px-4 rounded-full text-xs font-black uppercase tracking-widest w-fit mx-auto border ${
          ticket.is_used
        ...
        {/* Main ticket card */}
        <div className={`relative rounded-2xl overflow-hidden border ${ticket.is_used ? 'border-red-900/40' : 'border-zinc-800'} bg-[#0a0a0a] shadow-2xl`}>
          ...
          <div className="p-5">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div className="min-w-0">
                <h2 className="font-black uppercase tracking-widest text-white text-base leading-tight truncate">
                  {ticket.event?.title}
                </h2>
          ...
          <div className="px-5 pb-5 text-center">
            <p className="text-[9px] text-zinc-700 font-mono tracking-widest break-all">
              {ticket.token.slice(0, 24)}...
            </p>
          </div>
        </div>
